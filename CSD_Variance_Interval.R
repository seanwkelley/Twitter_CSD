...
tweet_type = "tweet"
path = paste0('Sentiments/',tweet_type,"/VADER_ANEW_LIWC_complete_dep.csv",collapse = "")
path2 = paste0('Sentiments/',tweet_type,"/CSD_Episodes.csv",collapse = "")

#sentiment analysis results based on tweet type 
FYP_df <- read.csv(path)
...
summary(lmer(SD ~ Time + WC + (1|Id),data =df))

summary(lmer(WC ~ Time + (1|Id),data =df))

ggplot(data = df, aes(x = SD,y=WC)) + geom_point() 
...
ggplot(data = df, aes(x = Time,y=WC)) + geom_boxplot() + theme_bw() + 
  scale_x_discrete(labels= c("14 days farthest from depression onset","14 days closest to depression onset")) +
