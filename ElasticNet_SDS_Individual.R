...
#############################################################
#############################################################
setwd('D:/Twitter_Study_FYP/Analysis2_Jan2020/')


enet <- read.csv('elasticNet/enet_10f_individual_l1_0.1.csv')

enet <- enet %>% select(c(X,SDS_20))
enet <- enet %>% filter(SDS_20 != 0)
enet <- enet %>% arrange(desc(abs(SDS_20))) %>% slice(1:10)
...
participants <- read.csv('Participant_Data/FYP_Twitter_Participants_03.03_v3.csv')
FYP_df <- read.csv('Sentiments/all_tweets/VADER_ANEW_LIWC_complete.csv')
