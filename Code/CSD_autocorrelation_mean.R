...
for(handle in id_list) {
  ...
  if(length(ex1$Day) >= 2*rolling_window) {
    ...
    #resid_series <- na.approx(ex1[,which(colnames(ex1) == var)])
    resid_series2 <- na.omit(ex1[,which(colnames(ex1) == var)])
    
    #rolling autocorrelation series using a pre-defined rolling window  
    autocor_resid <- acf.reference(resid_series2,rolling_window,1)
    #autocor_resid <- acf.reference(resid_series,length(resid_series),1)
    autocor_resid <- na.omit(autocor_resid)
    
    sd_resid <- na.omit(roll_sd(resid_series,rolling_window))
    #sd_resid <- na.omit(roll_sd(resid_series,length(resid_series)))
...
df <- as.data.frame(cbind(handle_list,mean_autocor,mean_sd,mean_sentiment,missing_vals))
colnames(df) <- c("Id","Autocor","SD","Sentiment","Percent_Missing")
...
#remove any outliers 
df[,2:5]= remove_all_outliers(df[2:5])
