...
VADER_ANEW_LIWC = pd.read_csv(path + 'Sentiments/' + tweet_type + '/' + 'VADER_ANEW_LIWC_complete.csv',encoding="utf-8")
...
for name in file_names:
    ...
    for start in start:
        ...
        missing_days.append(len(list(x for x in sentiments_handle['Day'] if (start-30) <= x <= (start - 1))))
...
CSD.to_csv('Participant_Data/CSD_06.04.csv',index = False)

#Participant_info['missing_days'] = start_dates
#Participant_info.to_csv('Participant_Data/Participants_14.02.csv',index = False)
